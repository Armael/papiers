...
(* Take [path], relative to the current working directory, and output
   the absolute path *)
let full_path_in_cwd (path: Fpath.t) =
...
let relative_path (db_path: Fpath.t) (path: Fpath.t) =
  match Fpath.relativize ~root:db_path path with
  | Some p -> p
  | None -> path

(* Relocate [path] to be relative to the database location [db_path] *)
let relocate (db_path: Fpath.t) (path: Fpath.t) =
  let relocated = path
    |> full_path_in_cwd
    |> relative_path db_path
    |> Fpath.normalize
  in
  if Fpath.is_abs relocated then (
...
(* Take [path], relative to the db location, and output the absolute
   path *)
let full_path_in_db (db_path: Fpath.t) (path: Fpath.t) =
  if Fpath.is_rel path then
    Fpath.append db_path path
  else
    path
...
let import_source
  ...
  |> Source.map_path (fun path ->
    if check_file_exists then (
      let full_path = full_path_in_cwd path |> Fpath.to_string in
